  ...
  try {
    ...
    switch (type) {
      ...
      case "analogy": {
        ...
        result = await generateListPrompt(instruction, ["部屋の片付け|料理"], [
          { step: 1, label: "要素抽出", placeholder: "課題とテーマの要素（例: 片付け＝捨てるのが辛い。料理＝完成図を決めて作る）" },
          { step: 2, label: "法則発見", placeholder: "共通する法則（例: 料理は完成図がある。片付けも「どんな部屋か」の完成図があれば捨てられるのでは？）" },
          { step: 3, label: "転用と解決", placeholder: "具体的なアクション（例: 『1週間後の理想の部屋の絵』を描き、合わない物は即捨てる）" }
        ], "他分野の仕組みを自分の課題に応用する力を鍛える。", "【聞かれていること】：今の課題を「別の何か」に置き換えた解決策。\n【答えるべきこと】：別の分野で似た構造を持つ事象と、そこから得られるヒント。\n【具体的な行動】：カレー作りと経営はどう似てる？違う世界から無理やり「成功パターン」をパクってこい！俺から先に答えは言わねえぞ。"); break;
      }
      case "metaphor-coach": {
        ...
        result = await generateListPrompt(instruction, ["時間", "ルール"], [
          { step: 1, label: "解剖", placeholder: "構成要素（例: ルール＝強制力、破ると怒られる、守ると安全、窮屈）" },
          { step: 2, label: "飛躍", placeholder: "要素から連想されるモノ（例: 透明な壁、ギプス、線路）" },
          { step: 3, label: "読解", placeholder: "伝えたい空気感（例: 窮屈だけど守ってくれる、というチクチクした感じ）" },
          { step: 4, label: "接合", placeholder: "AはBである（例: ルールは、人生の『透明なギプス』だ。）" },
          { step: 5, label: "洗練", placeholder: "情景をつけて完成（例: ルールってのは透明なギプスや。痒くて窮屈やが、外したらしこたま弱くなる）" }
        ], "芸術的なプロセスを経て、言語の鋭さを研ぎ澄ます。", "【聞かれていること】：平凡な言葉を「刺さる例え話」に進化させること。\n【答えるべきこと】：対象の分解、共通点の抽出、そして誰もが情景を思い浮かべる比喩。\n【具体的な行動】：俺が1段ずつステップを出すから、回答例をヒントに焦らず答えろ。最後に俺を唸らせる表現を作れ！"); break;
      }
      case "fogcatcher":
