...
シーンのみを出力してください。`;
      ...
      case 'whysos': {
        const defaultChallenge = ['最近感じたイライラの原因', '最近直面した仕事の課題', '最近成功したことの背景', '最近失敗したことの根本原因', '最近気になっていることの根本原因', '最近変えたい習慣の理由'][Math.floor(Math.random() * 6)];
...
- 深掘りが可能な課題であること`;
        ...
        // Parse the response - extract just the challenge line
        let challenge = defaultChallenge;
        const lines = text.split('\n').filter(line => line.trim());
        for (const line of lines) {
          if (!line.includes('【') && !line.includes('回答例') && !line.includes('トレーニングの目的') && !line.includes('制約条件') && line.length > 5) {
            challenge = line.replace(/^【課題】\s*/, '').replace(/^課題\s*/, '').trim();
            break;
          }
        }
      ...
      case 'sowhat': {
        const defaultTheme = ['最近読んだニュースの本質', '最近の仕事での出来事から得られる教訓', '最近の友人との会話の意味', '最近の失敗体験から学ぶべきこと', '最近観察した社会現象の背景'][Math.floor(Math.random() * 6)];
...
- 行動可能な教訓に落とし込むこと`;
        ...
        // Parse the response - extract just the theme line
        let theme = defaultTheme;
        const lines = text.split('\n').filter(line => line.trim());
        for (const line of lines) {
          if (!line.includes('【') && !line.includes('回答例') && !line.includes('トレーニングの目的') && !line.includes('制約条件') && line.length > 5) {
            theme = line.replace(/^【テーマ】\s*/, '').replace(/^テーマ\s*/, '').trim();
            break;
          }
        }
      ...
      case '5w1h': {
        const defaultTheme = ['最近の買い物の5W1H整理', '最近の会議の5W1H整理', '最近の旅行の5W1H整理', '最近のトラブル解決の5W1H整理', '最近のプロジェクトの5W1H整理'][Math.floor(Math.random() * 6)];
...
- 行動プランが明確であること`;
        ...
        result = {
          question: theme || '最近の買い物について5W1Hで整理して行動プランを作成してください。',
          steps: [
      ...
      case 'prep': {
        const defaultTheme = ['リモートワークのメリットをPREP法で伝える', '朝活の重要性をPREP法で伝える', '時間管理の重要性をPREP法で伝える', '継続の力をPREP法で伝える', 'デジタル detox の効果をPREP法で伝える'][Math.floor(Math.random() * 6)];
...
- 論理的に説得力があること`;
        ...
        result = {
          question: theme || 'リモートワークのメリットをPREP法で伝えてください。',
          steps: [
      ...
      case 'fogcatcher': {
        const defaultTheme = ['今の頭の中にある「ぼんやりとした悩み」', '最近感じた「漠然とした不安」', '最近のモヤモヤした感情', '言いにくい思考', '将来への不確かな思い'][Math.floor(Math.random() * 6)];
...
- 言語化することで思考を整理すること`;
        ...
        result = {
          question: theme || '今の頭の中にある「ぼんやりとした悩み」',
